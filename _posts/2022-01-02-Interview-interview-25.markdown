...
- 넘겨받은 쿠키는 <span style="background: rgb(251,243,219)">클라이언트가 가지고 있다가</span>(로컬 PC에 저장)다시 서버에 요청할 때 요청과 함께 쿠키를 전송한다.
- 동일 사이트 재방문 시 <span style="background: rgb(251,243,219)">클라이언트의 PC에 해당 쿠키가 있는 경우</span>, HTTP Header에 쿠키를 실어서 서버에 Request한다.
